...
var playState = {
    ...
    create: function () {
        ...
        // Create a score text
        this.scoreLabel = game.add.text(435, 40, '0x', {
    ...
    playerShoot: function () {
        if (game.time.now > nextShot && bullets.countDead() > 0 && this.player.alive == true) {
            ...
            var bullet = bullets.getFirstDead(); // getFirstDead tager det første døde objekt i gruppen, som bliver det objekt vi spawner.
    ...
    makeProblem: function () {
        var found = false;
        while (!found) {
            A = game.rnd.integerInRange(1, 4);
            B = game.rnd.integerInRange(0, 4);
            C = A + B;
            if (!(C > 4 || C < 1)) {
                found = true;
            }
    ...
    gameComplete: function () {
        if (this.gameWinSound.isPlaying == false) {    // Avoid spamming the function in update
            ...
            game.global.score = Math.round(((score / this.counter) * 60));
